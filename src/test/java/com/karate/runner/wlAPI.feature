...
Scenario: Create batch timings 
	...
	And request {"batchId":2747,"subjectId":549,"facultyId":453733,"timings":[{"day":0,"classTimings":[{"start":"19:40","end":"21:00"}]}]} 
...
Scenario: add students from app downloads 
	...
	And request {"studentContacts":[{"name":"testing stdnt","mobile":"919074569995"}]} 
...
Scenario: add assignee 
	...
	And header x-access-token = authToken 
	And header Content-Type = 'application/json' 
	And request {"name":"Assignee Name 1","contact":{"mobile":"9891302911","countryCode":"91"}} 
	When method POST 
	Then status 200 
	And def reponseMessage = response.message 
...
Scenario: Create Zoom Session 
	Given path '/v3/zoom/session' 
	And header x-access-token = authToken 
	And header Content-Type = 'application/json' 
	And request {"entityId": 453740,"entityType": 3} 
	When method POST 
	Then status 200 
	And def reponseMessage = response.message 
	And match reponseMessage == "Zoom session created successfully" 
